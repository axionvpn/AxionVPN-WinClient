...
CreatePopupMenus()
{
    ...
    if (o.num_configs == 1) {
      ...
      //  if (o.allow_edit[0] == '1')
		...
		AppendMenu(hMenu, MF_STRING, IDM_NETWORKS, _T("Networks"));
		AppendMenu(hMenu, MF_STRING, IDM_SETTINGS, LoadLocalizedString(IDS_MENU_SETTINGS));
        AppendMenu(hMenu, MF_STRING ,IDM_CONNECTIONS, LoadLocalizedString(IDS_MENU_CONNECTIONS));
		...
		//AppendMenu(hMenu, MF_STRING, IDM_NETWORKS, LoadLocalizedString(IDS_MENU_NETWORKS));
		AppendMenu(hMenu, MF_STRING, IDM_NETWORKS, _T("Networks"));
		AppendMenu(hMenu, MF_STRING, IDM_SETTINGS, LoadLocalizedString(IDS_MENU_SETTINGS));
        AppendMenu(hMenu, MF_STRING, IDM_CLOSE, LoadLocalizedString(IDS_MENU_CLOSE));
...
SetTrayIcon(conn_state_t state)
{
    ...
    if (CountConnState(connected) == 1) {
		...
		/* We don't care about assigned IP as much as remote IP
		...
		*/
		...
		//Account type
		WCHAR acctType[64];
		memset(acctType, 0, 64 * sizeof(WCHAR));
		swprintf(acctType, 64, L"\n%s %S", LoadLocalizedString(IDS_NFO_ACCOUNT_TYPE), o.conn[config].acctType);
		_tcsncat(msg, acctType, _countof(msg) - _tcslen(msg) - 1);
