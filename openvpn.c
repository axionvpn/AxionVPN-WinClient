...
void ClearCreds(HWND hwndDlg){
	...
	//Clear out the Stored values Also
	...
	SetRegistryValueBin(regkey, L"UserName", L"", 1 * sizeof(CHAR));
	SetRegistryValueBin(regkey, L"password", L"", 1 * sizeof(CHAR));

	RegCloseKey(regkey);

}

void SaveCreds(LPTSTR userName, LPTSTR password){
	HKEY regkey;
	DWORD dwDispos;
	...
	SetRegistryValueBin(regkey, L"UserName", userName,(strlen(userName)+1) * sizeof(CHAR));

	SetRegistryValueBin(regkey, L"password", password,(strlen(password)+1) * sizeof(CHAR));
...
/*
* DialogProc Storing Axion Credentials
*/
INT_PTR CALLBACK
AxionAuthDialogPopupFunc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	LPSTR userName;
	int userNameLen;
	LPSTR password;
	...
	case WM_INITDIALOG:
		...
		//If there are any creds Load them
...
*/
INT_PTR CALLBACK
AxionAuthDialogFunc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	connection_t *c;
	LPSTR userName;
	int userNameLen;
...
StopOpenVPN(connection_t *c)
{
	...
	//Get a handle to the thread we want to signal 
	//so we can wait for it
	  //HANDLE hThread = OpenThread(SYNCHRONIZE,FALSE,c->threadId);
	  //if(hThread){
		//  waitTime = INFINITE;
	  //}else{
	...
	//  WaitForSingleObject(hThread,waitTime);
